...
String interpretResult(int sec) {
  ...
  else if (uvAvg >= 3 && uvAvg < 6) { // for vit D: average 25 mins (1500 seconds) for total
  ...
  else if (uvAvg >= 6 && uvAvg < 8) { // for vit D: average 17 mins (1020 seconds) for total
    uvPercent = (sec/1020.0) * 100.0;
    if (sec >= 900 && sec <= 1500) { // 20 mins +- 5 min range
      burn = "There is a risk of sunburn. Apply sunscreen every 2 hours.";
    }
  }
  else if (uvAvg >= 8 && uvAvg < 10) { // for vit D: average 12 mins (720 seconds) for total
  ...
  else { // for vit D: average 7 mins (420 seconds) for total
